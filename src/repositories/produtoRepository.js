...
class ProdutoRepository {
  ...
  async listarProdutos(req) {
    ...
    if (id) {
      ...
      const data = await this.model.findById(id).populate("id_fornecedor");
    ...
    if (nome_fornecedor) {
      ...
      if (fornecedores.length > 0) {
        ...
        console.log(
          `Aplicando filtro por nome de fornecedor: "${nome_fornecedor}" (IDs: ${fornecedorIds.join(
            ", "
          )})`
        );
      } else {
        ...
        console.log(
          `Nenhum fornecedor encontrado com o nome: "${nome_fornecedor}"`
        );
        return {
          docs: [],
          totalDocs: 0,
  ...
  async atualizarProduto(id, dadosProduto) {
    ...
    console.log("Resultado da atualização:", produto);
