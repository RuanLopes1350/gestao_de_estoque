...
class MovimentacaoService {
  ...
  async buscarMovimentacoesPorPeriodo(
    dataInicio,
    dataFim,
    page = 1,
    limite = 10
    ...
    console.log(
      "Estou no buscarMovimentacoesPorPeriodo em MovimentacaoService"
    );
  ...
  async buscarMovimentacoesPorProduto(produto, page = 1, limite = 10) {
    console.log(
      "Estou no buscarMovimentacoesPorProduto em MovimentacaoService"
    );
  ...
  async buscarMovimentacoesPorUsuario(usuario, page = 1, limite = 10) {
    console.log(
      "Estou no buscarMovimentacoesPorUsuario em MovimentacaoService"
    );
    ...
    if (dadosAtualizacao.produtos && movimentacaoOriginal.tipo === "saida") {
      ...
      for (const produtoMov of movimentacaoOriginal.produtos) {
        try {
          ...
          console.error(
            `Erro ao restaurar estoque para produto ${produtoMov.produto_ref}:`,
            error
          );
        }
      }
      ...
      movimentacaoOriginal.tipo === "entrada"
    ) {
      ...
      for (const produtoMov of movimentacaoOriginal.produtos) {
        try {
          ...
          console.error(
            `Erro ao ajustar estoque para produto ${produtoMov.produto_ref}:`,
            error
          );
        }
      }
  ...
  async filtrarMovimentacoesAvancado(filtros = {}, opcoesPaginacao = {}) {
    ...
    return this.repository.filtrarMovimentacoesAvancado(
      filtros,
      opcoesPaginacao
    );
  }
