import { FornecedorSchema } from "../../FornecedorSchema.test.js";
...
describe("FornecedorSchema", () => {
  ...
  it("deve rejeitar nome_fornecedor muito curto", () => {
    ...
    expect(() => FornecedorSchema.parse(f)).toThrow(
  ...
  it("deve rejeitar telefone com tamanho incorreto", () => {
    ...
    expect(() => FornecedorSchema.parse(f)).toThrow(
      /Telefone deve conter exatamente 10 caracteres/
    );
  });
  ...
  it("deve rejeitar email muito longo", () => {
    ...
    expect(() => FornecedorSchema.parse(f)).toThrow(
      /Email deve conter no máximo 100 caracteres/
    );
  });
  ...
  it("deve rejeitar endereço com logradouro muito curto", () => {
    const f = {
      ...validFornecedor,
      endereco: [{ ...validFornecedor.endereco[0], logradouro: "AB" }],
    };
    expect(() => FornecedorSchema.parse(f)).toThrow(
  ...
  it("deve rejeitar endereço com bairro muito curto", () => {
    const f = {
      ...validFornecedor,
      endereco: [{ ...validFornecedor.endereco[0], bairro: "AB" }],
    };
    expect(() => FornecedorSchema.parse(f)).toThrow(
      /Bairro deve ter pelo menos 3 caracteres/
    );
  });
  ...
  it("deve rejeitar endereço com cidade muito curta", () => {
    const f = {
      ...validFornecedor,
      endereco: [{ ...validFornecedor.endereco[0], cidade: "AB" }],
    };
    expect(() => FornecedorSchema.parse(f)).toThrow(
  ...
  it("deve rejeitar endereço com estado diferente de 2 caracteres", () => {
    const f = {
      ...validFornecedor,
      endereco: [{ ...validFornecedor.endereco[0], estado: "S" }],
    };
    expect(() => FornecedorSchema.parse(f)).toThrow(
  ...
  it("deve rejeitar endereço com cep não numérico", () => {
    const f = {
      ...validFornecedor,
      endereco: [{ ...validFornecedor.endereco[0], cep: "1234abcd" }],
    };
    expect(() => FornecedorSchema.parse(f)).toThrow(
